...
import { parseSnapshot, storePatchFromSnapshot } from "../../src/lib/project-file";
...
test.describe("DAS-139 parseSnapshot validation", () => {
  ...
  test("minimal valid snapshot applies store defaults on load patch", () => {
    ...
    expect(patch.costScenarios).toEqual([]);
    expect(patch.subgradeCbr).toBe(8);
    expect(patch.cesaResult).toBeNull();
  });
});
